  ...
  const handleClearFieldFacet = (key, value) => {
    return () => {
      if (query.selectedFieldFacets[key]) {
        const selected = produce(query.selectedFieldFacets[key], (selectedDraft) => {
  ...
  const handleClearQueryFacet = (key, value) => {
    return () => {
      if (query.selectedQueryFacets[key]) {
        const selected = produce(query.selectedQueryFacets[key], (selectedDraft) => {
          const index = selectedDraft.indexOf(value);
          if (index !== -1) {
            selectedDraft.splice(index, 1);
          }
  ...
  if (displayData.includes('facets')) {
    for (const key in query.fieldFacets) {
      if (query.selectedFieldFacets[key] && query.selectedFieldFacets[key].length > 0) {
        filters.push(
          <Typography key={nextId()} component="span">
            <Typography component="span" color="secondary">
              {query.fieldFacets[key].title}:&nbsp;
            </Typography>
            {query.selectedFieldFacets[key].map((entry, index) => (
              <FilterEntry key={index} value={entry} onClick={handleClearFieldFacet(key, entry)} />
            ))}
          </Typography>
        );
      }
        ...
        filters.push(
          <Typography key={nextId()} component="span">
            ...
            {query.selectedQueryFacets[key].map((entry) => (
              <FilterEntry value={entry} onClick={handleClearQueryFacet(key, entry)} />
            ))}
          </Typography>
        );
      }
